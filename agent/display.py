    ...
    *args*, falls back to showing only the result.  A status icon (✅ success
    ...
    if tool_name == "cronjob":
        ...
        if action == "create":
            ...
            return _fmt(f"┊ ⏰ cron      create {_trunc(label, 24)}  {dur}")
        if action == "list":
            return _fmt(f"┊ ⏰ cron      listing  {dur}")
        return _fmt(f"┊ ⏰ cron      {action} {args.get('job_id', '')}  {dur}")
    if tool_name == "execute_code":
        code = args.get("code", "")
        first_line = code.strip().split("\n")[0] if code.strip() else ""
        return _fmt(f"┊ 🐍 exec      {_trunc(first_line, 35)}  {dur}")
    if tool_name == "browser_exec":
        label = _browser_exec_step_label(args)
        if label is not None:
    ...
    if tool_name == "delegate_task":
        ...
        if tasks and isinstance(tasks, list):
            ...
            return _fmt(f"┊ 🔀 delegate  {count_label}x: {_trunc(detail, 35)}  {dur}")
        return _fmt(f"┊ 🔀 delegate  {_trunc(args.get('goal', ''), 35)}  {dur}")
